...
try {
  console.log("🔄 Populando catálogo Veiling...");
  ...
  // 3. Obter catálogo Veiling
  const [catalogos] = await conn.execute(
    "SELECT id FROM catalogos_venda WHERE nome LIKE '%veiling%' LIMIT 1"
  );
  
  if (catalogos.length === 0) {
    console.log("❌ Catálogo Veiling não encontrado");
    await conn.end();
    process.exit(1);
  }

  const catalogoId = catalogos[0].id;
  ...
  // 5. Limpar catálogo existente
  const [deleteResult] = await conn.execute(
    "DELETE FROM catalogos_venda_itens WHERE catalogoId = ?",
    [catalogoId]
  );
  console.log(`🗑️  ${deleteResult.affectedRows} itens removidos do catálogo`);

  // 6. Inserir produtos no catálogo
  let inseridos = 0;
  let ordem = 0;
  ...
  for (const prod of veilingProdutos) {
    try {
      ...
      if (custoBase <= 0) {
        console.log(`⚠️  Pulando ${prod.nomeCompleto} (sem preço válido)`);
        continue;
      }
